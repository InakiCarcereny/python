...
def agregar_libro():
    if request.is_json:
        ...
        try:
            nuevo_libro = Libro.from_diccionario(datos_libro)
...
def actualizar_libro(ISBN):
    ...
    try:
        libro_existente = repo.obtener_libro(ISBN)
        if not libro_existente:
            return jsonify({"mensaje": "Libro no encontrado"}), 404

        if repo.actualizar_libro(ISBN, datos_actualizados):
            libro_final = repo.obtener_libro(ISBN)
            return jsonify({
...
def eliminar_libro(ISBN):
    try:
        ...
        if repo.eliminar_libro(ISBN):
            return jsonify({"mensaje": "Libro eliminado correctamente"}), 200
        else:
            return jsonify({"mensaje": "No se pudo eliminar el libro"}), 400
