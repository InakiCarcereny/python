  ...
  def ejecutar():
    try:
      ...
      for _ in range(cantidad_iteraciones):
        ...
        valor_random = random.randint(0, 50)

        if numero_iteracion == 0:
          print(f"velocidad: {automovil.obtener_velocidad_actual()}")
          print("acelerar")
          automovil.acelerar(valor_random)
          print(f"velocidad: {automovil.obtener_velocidad_actual()}")
        elif numero_iteracion == 1:
          print(f"velocidad: {automovil.obtener_velocidad_actual()}")
          print("desacelerar")
          automovil.desacelerar(valor_random)
          print(f"velocidad: {automovil.obtener_velocidad_actual()}")
          ...
          velocidad = automovil.calcular_minutos_para_llegar(valor_random)

          if (velocidad > 0):
            print(f"El tiempo para llega es de {velocidad:.2f} minutos")
          else:
